# 🏗️ ASMO-01 - Architecture détaillée

## 📐 Vue d'ensemble du système

```
┌─────────────────────────────────────────────────────────────────────┐
│                         ASMO-01 Server                               │
│  ┌────────────────────────────────────────────────────────────┐    │
│  │                    Docker Containers                        │    │
│  │  ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐   │    │
│  │  │n8n   │ │Jellyfin│Vault │ │Grafana│Home  │ │Homarr│   │    │
│  │  │      │ │        │warden│ │       │Assist│ │      │   │    │
│  │  └──────┘ └──────┘ └──────┘ └──────┘ └──────┘ └──────┘   │    │
│  │  ... (24 containers total)                                 │    │
│  └────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  ┌────────────────────────────────────────────────────────────┐    │
│  │              Health Monitoring System                       │    │
│  │  /home/scripts/asmo-health/                                │    │
│  │  ├── src/                                                   │    │
│  │  │   ├── monitor.py    (hourly collection)                │    │
│  │  │   ├── reporter.py   (daily analysis)                   │    │
│  │  │   └── utils/        (Docker + System metrics)          │    │
│  │  ├── data/                                                  │    │
│  │  │   └── health_history.json  (7 days retention)          │    │
│  │  └── logs/                                                  │    │
│  │      └── asmo.log                                           │    │
│  └────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────┘
                              ▲
                              │ SSH Commands
                              │
                    ┌─────────┴─────────┐
                    │     n8n Workflows  │
                    │   (Automation)     │
                    └─────────┬─────────┘
                              │
                              ▼
                    ┌────────────────────┐
                    │  Discord Webhook   │
                    │   (Notifications)  │
                    └────────────────────┘
```

---

## 🔄 Flux de données - Monitoring Horaire

```
┌────────────┐
│  n8n Cron  │  Every hour: 0 * * * *
└──────┬─────┘
       │
       ▼
┌─────────────────────────────────────────────────────────┐
│  monitor.py                                              │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 1. Collect System Metrics                         │  │
│  │    • CPU usage (psutil)                           │  │
│  │    • RAM usage (psutil)                           │  │
│  │    • Disk usage (psutil)                          │  │
│  │    • Network I/O (psutil)                         │  │
│  │    • System uptime                                │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 2. Collect Docker Metrics                         │  │
│  │    • Container list (docker.py API)               │  │
│  │    • Container status (running/stopped/unhealthy) │  │
│  │    • CPU & RAM per container                      │  │
│  │    • Network I/O per container                    │  │
│  │    • Restart count                                │  │
│  │    • Recent error logs (last 50 lines)            │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 3. Analyze Thresholds                             │  │
│  │    • Check CPU > 95% (critical)                   │  │
│  │    • Check RAM > 95% (critical)                   │  │
│  │    • Check disk > 90% (critical)                  │  │
│  │    • Check unhealthy containers                   │  │
│  │    • Check restart counts                         │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 4. Store in History                               │  │
│  │    data/health_history.json                       │  │
│  │    [                                              │  │
│  │      { timestamp: "...", metrics: {...} },        │  │
│  │      { timestamp: "...", metrics: {...} },        │  │
│  │      ...                                           │  │
│  │    ]                                               │  │
│  │    Auto-cleanup: Keep last 7 days                 │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 5. Return JSON Output                             │  │
│  │    {                                               │  │
│  │      "success": true,                              │  │
│  │      "critical_alert": false,                      │  │
│  │      "summary": {...}                              │  │
│  │    }                                               │  │
│  └───────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
       │
       ▼
┌────────────────────────────────────────┐
│  Optional: IF critical_alert == true   │
│  → Send immediate Discord alert        │
└────────────────────────────────────────┘
```

---

## 📊 Flux de données - Rapport Journalier

```
┌────────────┐
│  n8n Cron  │  Daily at 9:00 AM: 0 9 * * *
└──────┬─────┘
       │
       ▼
┌─────────────────────────────────────────────────────────┐
│  reporter.py                                             │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 1. Load 24h History                               │  │
│  │    storage.get_last_24h()                         │  │
│  │    → Returns ~24 entries (hourly snapshots)       │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 2. Analyze Trends                                 │  │
│  │    • CPU: avg, min, max over 24h                  │  │
│  │    • RAM: avg, min, max over 24h                  │  │
│  │    • Disk: trends                                 │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 3. Analyze Container Health                       │  │
│  │    For each container:                            │  │
│  │    • Calculate uptime % (running vs down checks)  │  │
│  │    • Aggregate restart counts                     │  │
│  │    • Collect unique errors                        │  │
│  │    • Calculate avg/max resource usage             │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 4. Identify Problems                              │  │
│  │    • Containers with uptime < 100%                │  │
│  │    • Containers with restarts                     │  │
│  │    • Containers with errors                       │  │
│  │    • Severity: critical / warning / info          │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 5. Get Top Resource Consumers                     │  │
│  │    • Top 5 by memory usage                        │  │
│  │    • Top 5 by CPU usage                           │  │
│  └───────────────────────────────────────────────────┘  │
│                                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 6. Generate Discord Embed                         │  │
│  │    {                                               │  │
│  │      "embeds": [{                                  │  │
│  │        "title": "✅ ASMO-01 • 24h Report",         │  │
│  │        "fields": [                                 │  │
│  │          { "name": "Current State", ... },         │  │
│  │          { "name": "Issues", ... },                │  │
│  │          { "name": "Top Memory", ... },            │  │
│  │          { "name": "Top CPU", ... }                │  │
│  │        ]                                           │  │
│  │      }]                                            │  │
│  │    }                                               │  │
│  └───────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
       │
       ▼
┌────────────────────────────────────────┐
│  n8n HTTP Request Node                 │
│  POST to Discord Webhook               │
└──────────────┬─────────────────────────┘
               │
               ▼
         ┌──────────────┐
         │   Discord    │
         │   Channel    │
         └──────────────┘
```

---

## 🗄️ Structure des données

### Entrée d'historique (health_history.json)

```json
{
  "timestamp": "2026-01-11T10:00:00.123456",
  "cpu_percent": 15.3,
  "cpu_count": 8,
  "ram_total_gb": 16.0,
  "ram_used_gb": 5.5,
  "ram_percent": 34.4,
  "swap_total_gb": 4.0,
  "swap_used_gb": 0.0,
  "disks": [
    {
      "mount": "/",
      "device": "/dev/nvme0n1p2",
      "total_gb": 457.0,
      "used_gb": 115.0,
      "used_percent": 27.0
    },
    {
      "mount": "/mnt/nas",
      "device": "/dev/sda1",
      "total_gb": 3600.0,
      "used_gb": 1100.0,
      "used_percent": 31.0
    }
  ],
  "uptime": "15d 7h 23m",
  "load_average": [1.5, 1.3, 1.2],
  "containers": [
    {
      "name": "jellyfin",
      "id": "abc123",
      "status": "running",
      "state": "running",
      "health": "healthy",
      "cpu_percent": 0.21,
      "mem_mb": 308.2,
      "mem_percent": 1.9,
      "net_rx_mb": 13.3,
      "net_tx_mb": 1.58,
      "restarts": 0,
      "errors": [
        "SQLite Error 5: database is locked",
        "..."
      ]
    }
  ],
  "containers_total": 24,
  "containers_running": 24,
  "containers_stopped": 0,
  "containers_unhealthy": 0,
  "alert": {
    "has_critical": false,
    "critical_issues": [],
    "warnings": []
  }
}
```

---

## 🔌 Intégrations externes

### Docker Socket
```
/var/run/docker.sock
   ↓ (unix socket)
DockerMonitor (docker.py client)
   ↓
Container stats, logs, health
```

### Système Linux
```
psutil library
   ↓
/proc filesystem
   ↓
CPU, RAM, Disk, Network metrics
```

### n8n Workflows
```
Schedule Trigger (cron)
   ↓
Execute Command (SSH)
   ↓
Python scripts
   ↓
JSON output
   ↓
HTTP Request (Discord)
```

---

## 🔐 Sécurité & Permissions

### Permissions requises

```
Docker Socket:
  • User doit être dans le groupe 'docker'
  • Permission lecture sur /var/run/docker.sock

Filesystem:
  • Lecture: /proc (psutil)
  • Écriture: /home/scripts/asmo-health/data/
  • Écriture: /home/scripts/asmo-health/logs/

Network:
  • Sortant uniquement (vers Discord webhook)
  • Pas de port listen
```

### Données sensibles

```
config.json (gitignored):
  • Discord webhook URLs
  • Chemins locaux
  • Seuils personnalisés

health_history.json (gitignored):
  • Noms de containers
  • Métriques système
  • Logs d'erreurs (peuvent contenir des infos)
```

---

## 📈 Scalabilité

### Limites actuelles

```
Containers: Testé jusqu'à 30 containers
History:    7 jours × 24 entrées = ~168 entrées
File size:  ~500 KB - 2 MB (selon verbosité des erreurs)
RAM usage:  ~50-100 MB pendant l'exécution
CPU usage:  ~5-10% pendant 2-3 secondes
```

### Pour scaler

```
Plus de containers (50+):
  → Augmenter retention_days si besoin
  → Filtrer les containers non-critiques

Monitoring plus fréquent (15 min):
  → 4× plus d'entrées
  → Réduire retention_days à 3-4 jours

Multi-serveurs:
  → Déployer sur chaque serveur
  → Centraliser les rapports (future feature)
```

---

## 🔄 Cycle de vie des données

```
Collection (hourly)
   ↓
health_history.json
   ↓
Retention: 7 days
   ↓
Auto-cleanup (when adding new entry)
   ↓
Old entries deleted

Logs (continuous)
   ↓
asmo.log
   ↓
Rotation: Manual (future: logrotate)
```

---

## 🎯 Points d'extension

### Ajouter une nouvelle métrique

1. **Dans `metrics.py`**: Ajouter fonction de collecte
2. **Dans `monitor.py`**: Appeler la fonction, stocker dans dict
3. **Dans `reporter.py`**: Utiliser dans l'analyse
4. **Dans embed**: Afficher dans Discord

### Ajouter un nouveau type de rapport

1. Créer nouveau script `src/my_report.py`
2. Importer `HealthStorage` pour lire history
3. Générer le format de sortie voulu (JSON, HTML, etc.)
4. Créer workflow n8n pour l'exécuter

### Intégrer un nouveau service

1. **Grafana**: Exporter metrics vers Prometheus format
2. **API**: Wrapper Flask autour de HealthStorage
3. **Webhook**: Poster vers endpoint custom au lieu de Discord

---

Voilà l'architecture complète ! 🏗️✨
